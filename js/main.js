(function () {
  ...
  function initializePlayerNames() {
    ...
    if (isComputerMode) {
      document
        .querySelector(".player-name-container")
        .classList.add("single-player");
      document.getElementById("player1Name").placeholder = "Enter your name";
    } else {
      document
        .querySelector(".player-name-container")
        .classList.remove("single-player");
      document.getElementById("player1Name").placeholder = "Player 1 name";
    }
  }

  document.getElementById("saveNames").addEventListener("click", function () {
    const newName = document.getElementById("player2Name").value.trim();
    if (newName) {
      player2Name = newName;
  ...
  function updateLabels() {
    ...
    if (isComputerMode) {
      ...
      player2Label.textContent = `${playerName}'s Score`;
    } else {
      player1Label.textContent = `${player1Name}'s Score`;
      player2Label.textContent = `${player2Name}'s Score`;
    }
  }
  ...
  function dotClick(dotIndex) {
    ...
    if (t2 != 0 && t2 != t1) {
      ...
      if (matched.length > 0) {
        ...
        if (
          _.some(target.classList, (cls) => cls === "blue" || cls === "red")
        ) {
  ...
  function go(t) {
    if (occ[t] != 1) {
      ...
      if (isComputerMode) {
        ...
        target.classList.add(currentPlayer === 1 ? "blue" : "red");
      }
